...
create {ETF_SD_MAIN_MODEL_ACCESS}
	make
...
feature -- model operations
		...
		do
			...
			current_projectile_moves:= 0
				...
				if is_player_dead then
					projectile_pos.go_i_th(removed_index)
	...
	set_message(c: STRING)
		...
		do
			command_name:= c
			message:= ""
		if not (cursor_position > 0 and cursor_position <= message_array.count) then
			...
			elseif command_name ~ "move" then
				projectile_message
				if removed_index > projectile_pos.count and is_player_dead then
							message.append ("  ")
							message.append ("A projectile moves: [" + arr.at (element_removed.r).out)
							message.append("," + (element_removed.c - projectile_max_moves).out + "] -> [")
							message.append (arr.at (element_removed.r).out + ",")
							message.append ((element_removed.c.out) + "]")
							message.append ("%N")
				end
				if player_move_row = element_removed.r and player_move_col = element_removed.c then
					message.append ("  ")
		...
		do
			...
			loop
					if command_name ~ "fire" then
						if x < projectile_pos.count then
						...
						if removed_index = x and command_name ~ "move" and is_player_dead then
							message.append ("  ")
							message.append ("A projectile moves: [" + arr.at (element_removed.r).out)
							message.append("," + (element_removed.c - projectile_max_moves).out + "] -> [")
							message.append (arr.at (element_removed.r).out + ",")
